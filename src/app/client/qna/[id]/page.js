...
지금 6세면 아직 충분히 개선 가능한 시기입니다. 너무 걱정하지 마세요!`,
  ...
  const handleLikeQuestion = async () => {
    try {
      // await QnAAPI.likeQuestion(question.id);
      setQuestion({
        ...question,
        is_liked: !question.is_liked,
        likes_count: question.is_liked ? question.likes_count - 1 : question.likes_count + 1
      });
    } catch (error) {
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                  <Button
                    ...
                    className={question.is_liked ? 'text-red-500' : ''}
                  >
                    <Heart className={`mr-1 h-4 w-4 ${question.is_liked ? 'fill-current' : ''}`} />
